...
1. Install https://adoptium.net/temurin/releases[JDK 25] (pom requires Java 25).
2. Install http://maven.apache.org/download.cgi[Apache Maven], *or* use a version manager:
   * https://mise.jdx.dev/[mise] — `mise install` (see `mise.toml`)
   * https://sdkman.io/[sdkman] — `sdk env` (see `.sdkmanrc`)
3. From the repo root:
+
----
mvn -DskipTests -Plocal-run spring-boot:run
----
+
The `local-run` profile is **declarative**: it pulls OS-native OpenJFX jars from Maven Central into `target/javafx-mods` and wires `--module-path` for you. No Gluon zip and no permanent JavaFX entries on your user `PATH`.
+
Helpers: `scripts/afx-run.ps1` (Windows) / `scripts/afx-run.sh` (Unix).
+
Fork status (custom title bar **unsolved**, native chrome kept so the window can move): see `FORK.md`.
4. Packaged scripts (optional): `mvn -DskipTests package -Pinstall4j-package` then look under `target/appassembler/bin` for `asciidocfx` / `asciidocfx.bat`.
